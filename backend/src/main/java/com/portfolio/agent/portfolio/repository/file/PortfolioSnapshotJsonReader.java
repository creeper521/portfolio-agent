...
public final class PortfolioSnapshotJsonReader {
    ...
    public PortfolioSnapshot readLegacyResource(byte[] bytes) {
        try {
            ...
            requireExactFields(root, LEGACY_FIELDS, "legacy portfolio resource field set is not canonical");
            root.set("questionPresets", root.remove("questions"));
            root.set("timelineEvents", root.remove("timeline"));
            root.putArray("cases");
            normalizeMissingArray(root.withArray("questionPresets"), "caseIds");
            normalizeMissingArray(root.withArray("timelineEvents"), "caseIds");
            return strictMapper.treeToValue(root, PortfolioSnapshot.class);
