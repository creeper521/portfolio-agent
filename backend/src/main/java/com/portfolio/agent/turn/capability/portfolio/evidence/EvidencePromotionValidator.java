 ...
 * 校验主体类型属于本次 Invocation 的允许集合，并对 claimId + evidenceCode 做整批去重。
 * 任何一条不满足都整批作废（fail-closed），绝不部分放行。
 */
public final class EvidencePromotionValidator {
     ...
     * @param allowedSubjectKinds      本次调用冻结的非空主体类型集合
    ...
    public ValidatedEvidenceBundle promote(
        ...
        if (allowedSubjectKinds == null || allowedSubjectKinds.isEmpty()
                || allowedSubjectKinds.stream().anyMatch(Objects::isNull)) {
        ...
        candidateSet.getSubjects().forEach(subject -> {
            if (!allowedSubjectKinds.contains(subject.getSubjectKind())) {
                throw new PortfolioEvidenceCapability.PortfolioCapabilityException(
                        PortfolioEvidenceCapability.IntegrityReason.SUBJECT_KIND_NOT_ALLOWED);
            }
            subject.getCandidates().forEach(candidate -> {
                ...
                units.add(new ValidatedEvidenceUnit(
                        ...
                        subject.getTitle(), subject.getCareerTrack(),
