...
Optional model operations and local public retrieval remain disabled unless explicitly configured. Model execution has three fail-closed startup authorities: `model-runtime`, each configured Provider's enabled/selectable/credential/data-policy admission, and each Operation's exact schema, output budget, and timeout. Every Turn explicitly selects `MODEL(modelRef + selectionVersion)` or `NONE`; one credential-free execution snapshot is frozen after Claim and shared by all model stages, with no implicit Provider fallback. The closed GLM/Qwen protocol profiles and local BGE path are admitted only under their approved privacy and configuration gates. Public PostgreSQL projection and private governance import remain separate, explicitly operated capabilities.
