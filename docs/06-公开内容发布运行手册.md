...
当前本地运行时基线为 schema 4.0、内容版本 `2026-07-29.1` 的七文件 Bundle：5 个 Project、49 个 Case、3 个 Collection、79 个 Claim、59 个 APPROVED Evidence、79 条 Claim–Evidence 关联、79 个检索 chunk、11 条 TimelineEvent、16 个 QuestionPreset；61/68 项资产公开，7 项 `EXCLUDE` 保持私有。该本地发布与导入记录不构成生产部署、真实 Provider 调用或线上验收通过的声明。
...
当前架构/E2E 门禁状态、CI 执行面、发布证据绑定、HTTP 安全响应头、SBOM/依赖/Secret/镜像扫描、超时任务资源回收和真实环境容量验证等观察，集中记录在 [`12-工程质量与未来优化评审备忘录.md`](12-工程质量与未来优化评审备忘录.md)。它们不改变本手册的现行命令契约；只有确认实施并完成验证后，才应加入正式发布步骤或阻断条件。
