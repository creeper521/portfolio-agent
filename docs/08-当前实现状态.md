...
- 作品集智能检索装配在 `portfolio.database.public.enabled=true` 时将 PostgreSQL/pgvector 设为主检索，并在其抛出 `PortfolioRetrievalException` 时自动改用 Bundle；降级结果固定标记为 `degraded=true` 与 `POSTGRES_RETRIEVAL_UNAVAILABLE`。数据库关闭或未配置时只暴露 Bundle 检索 seam，任一配置下外部只能注入一个 `PortfolioRetriever`。
