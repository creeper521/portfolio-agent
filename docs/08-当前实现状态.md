...
- AI 降级（`FALLBACK`）作为正常响应展示回答、降级提示和引导问题，可连续对话；仅传输失败进入重试视图，并同时提供三个本地公开问题。
- 对话消息可展示后端返回的结构化 `portfolioRecommendation`；前端按后端既定顺序渲染并在当前标签页内原样回传推荐上下文，刷新、关闭页面或新建会话后清空，后端每次重新校验。

### 2.2 公开内容与 API

- Spring Boot 提供 `GET /api/v1/portfolio`、`GET /api/v1/projects/{slug}`、`GET /api/v1/cases`、`GET /api/v1/cases/{slug}`、`GET /api/v1/public-content`、`POST /api/v1/answers` 和 `POST /api/v2/answers`。其中 `/api/v2/answers` 是唯一公开 Agent 入口；独立 Selection HTTP 接口已移除，推荐只作为 Agent 回答能力存在。
...
- 本地开发新增 `scripts/start-local.ps1`：只接受仓库外绝对 Secret 路径，预检六项 AI 配置、Java 21、Maven、Node、前端依赖和端口，托管本次创建的前后端进程，并用固定公开问题验证真实 `MODEL` 回答。根目录 `.env` 不会被隐式加载；只有探针通过才报告 `AI_CONNECTED`，否则明确报告安全降级类别。
...
- 发布门禁支持 `-RequireLiveProvider`：缺少审批、显式 Provider 或密钥时失败；普通模式以高优先级关闭模型能力，避免继承本机环境后误触外部调用。成功摘要只包含 Provider、内容版本、generation mode、resolution 和 block 数量。
...
### 2.12 Agent 作品集硬路由与无状态推荐

`/api/v2/answers` 已实际调用统一的 `PortfolioTaskResolver` 和 `PortfolioIntelligence`。确定性作品集请求、Project/Case 上下文、`recommendationContext`，以及外层意图路由识别出的作品集/混合问题，都会在通用生成模型之前进入该模块。事实和比较使用统一 retriever 返回的公开材料；推荐及调整使用确定性选择策略，响应保留完整结构化推荐和可复验上下文。

一般对话、时效问题与安全拒答保持原链路。作品集结果采用确定性 block 组装，模型不会增删、替换或重排推荐项；澄清只返回一个关键问题，合法空推荐仍返回结构化对象。数据库开关两种模式均提供唯一 intelligence/runtime 装配，诊断只记录封闭枚举、计数、上下文存在性、验证结果和耗时档位。

Project/Case slug 仅用于请求边界校验，运行时会将其解析为稳定公开作品 ID；hint 本身不再猜测任务模式。每个作品集硬路由最多执行一次受约束 `portfolio_task` 分类，同一结构化结果互斥表达 unsafe/time 边界或任务模式；外层判为 `PORTFOLIO_GROUNDED` 或 `HYBRID` 后也消费同一决策，boundary 直接安全降级，task 原样进入 intelligence，不再内部二次 resolve。模型边界可以抢占，确定性推荐、比较或调整规则优先于模型普通任务结果，未命中规则时才采用高置信模型任务。Provider 禁用时，确定性任务照常执行，hint-only 等未命中请求返回澄清且零模型调用。该 ID 只对单主体事实查询形成精确检索约束，推荐、推荐调整和比较仍在全局公开候选中计算，Bundle 与 PostgreSQL 路径遵守同一规则。确定性安全/时效判断和未知主体校验仍先于任何模型调用，诊断只记录封闭枚举、状态和计数，不记录原问题。`HYBRID` 路径会把 intelligence 材料映射为 grounding 后再生成和验证，成功保持混合意图与范围；澄清、无证据、模型或验证失败时返回同一批确定性作品材料，并把范围降为 `PORTFOLIO`。
...
| 对话式 Agent v2 后端 | `/api/v2/answers`、意图路由、通用/作品集/混合回答、公开检索、固定工具、事实校验和动态追问已实现，默认关闭；它是唯一公开 Agent 入口。上下文只由当前标签页内存随请求传递，刷新清空，后端无状态 | 生产启用还要求访客数据条款审批、单 Provider 密钥与线上验收 |
