...
Project/Case slug 仅用于请求边界校验，运行时会将其解析为稳定公开作品 ID；该 ID 只对单主体事实查询形成精确检索约束，推荐、推荐调整和比较仍在全局公开候选中计算，Bundle 与 PostgreSQL 路径遵守同一规则。安全和时效门禁复用外层路由的单一确定性 seam：先执行确定性判断，作品集硬路由再允许受约束模型仅返回 unsafe/time 边界；普通分类结果或 Provider 失败不会阻断硬路由，也不维护第二套词典。`HYBRID` 路径会把 intelligence 材料映射为 grounding 后再生成和验证，成功保持混合意图与范围；澄清、无证据、模型或验证失败时返回同一批确定性作品材料，并把范围降为 `PORTFOLIO`。
