...
Project/Case slug 仅用于请求边界校验，运行时会将其解析为稳定公开作品 ID；hint 本身不再猜测任务模式。每个作品集硬路由最多执行一次受约束 `portfolio_task` 分类，同一结构化结果互斥表达 unsafe/time 边界或任务模式：模型边界可以抢占，确定性推荐、比较或调整规则优先于模型普通任务结果，未命中规则时才采用高置信模型任务。Provider 禁用时，确定性任务照常执行，hint-only 等未命中请求返回澄清且零模型调用。该 ID 只对单主体事实查询形成精确检索约束，推荐、推荐调整和比较仍在全局公开候选中计算，Bundle 与 PostgreSQL 路径遵守同一规则。确定性安全/时效判断和未知主体校验仍先于任何模型调用，诊断只记录封闭枚举、状态和计数，不记录原问题。`HYBRID` 路径会把 intelligence 材料映射为 grounding 后再生成和验证，成功保持混合意图与范围；澄清、无证据、模型或验证失败时返回同一批确定性作品材料，并把范围降为 `PORTFOLIO`。
