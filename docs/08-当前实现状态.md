...
Project/Case slug 仅用于请求边界校验，运行时会将其解析为稳定公开作品 ID，并作为统一检索的精确主体约束；Bundle 与 PostgreSQL 路径遵守同一约束。安全和时效门禁复用外层路由的单一确定性 seam，优先于所有作品集硬路由。`HYBRID` 路径会把 intelligence 材料映射为 grounding 后再生成和验证，成功保持混合意图，失败则返回同一材料的确定性安全结果。
