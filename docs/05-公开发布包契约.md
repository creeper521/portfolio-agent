...
**日期：** 2026-07-15
**状态：** 当前运行时为 schema 4.0、内容版本 `2026-07-29.1` 的七文件 Bundle：5 个 Project、49 个 Case、3 个 Collection、79 个 Claim、59 个 APPROVED Evidence、79 条 Claim–Evidence 关联、79 个检索 chunk、11 条 TimelineEvent、16 个 QuestionPreset，公开 61/68 项资产，7 项 `EXCLUDE` 保持私有。B 四文件包与 C2a 七文件 retrieval 扩展均已实现并验证；旧包兼容，C2 只读工具/引用式多轮不属于本契约完成范围。
...
> 以下标为 schema 2.0 / `2026-07-20.1` 的 JSON 仅为**历史示例**，不描述当前运行时快照；当前发布以 schema 4.0、内容版本 `2026-07-29.1` 为准。
...
## 13. Preset Contract 评审观察入口

QuestionPreset 的 DRAFT 展示过滤、Project/Case 推荐投影、展示主体与执行主体、Case 页面 Contract 身份传递，以及 `questionPresetId`/`contractVersion` 原子性等阶段性观察，集中记录在 [`12-工程质量与未来优化评审备忘录.md`](12-工程质量与未来优化评审备忘录.md)。这些观察尚未修改本契约；决定实施时必须先明确兼容策略并形成单独设计。
