...
当前公开版本已完成 Keyword、Vector、Hybrid 三路真实本地模型 Wave 0 离线比较：12 个公开用例、36 条路线结果均经过主体、安全和隐私复核，完整指标与失败用例见 `docs/reports/retrieval-baseline-2026-07-24.md`。这是扩充真实公开项目与 Evidence 前的基线，不代表已证明 Hybrid 最终价值；运行时 Profile 未改变，检索仍默认关闭，也没有部署。
...
| C2a 本地检索 | 检索、索引、模型校验、三路离线比较与 Wave 0 真实模型基线已实现，运行时默认关闭 | 扩充真实公开项目、Evidence 与独立 holdout 后复测；生产侧仍需安装固定 revision 的本地 ONNX 模型并显式配置 `HYBRID`，当前 Git 不包含模型二进制 |
