...
后续采用“内容先行、逐项目纵向交付”的推进方式，不优先扩展通用 Agent 平台，也不先投入部署。实施顺序固定为：

1. 真实内容扩充与治理；
2. 每个项目同时完成页面视觉呈现与 Agent 问答；
3. 作品集整体主路径与跨项目能力整合；
4. 质量与可信度收口；
5. 可选智能能力评估；
6. 云服务器部署与求职交付。

内容准备完成后，不把“全部前端”和“全部 Agent”拆成两个长期分支。每个代表项目作为一个纵向批次，在同一批次内交付公开数据、项目页面、视觉呈现、QuestionPreset、Agent 回答、追问和证据链，再进入下一个项目。前一阶段未达到验收标准时，不提前用后续阶段掩盖内容或可信度缺口；不得用模型生成、视觉包装或动态 Agent 抽象替代真实项目、职责边界和公开证据。
...
先完成真实项目、Claim、Evidence 和候选问题的筛选与审核。之后按项目逐个同时完善前端页面和 Agent，而不是先做完全部页面再补 Agent，或反过来。该方案能让每个批次形成可演示、可验证的完整案例，并及时发现公开数据、视觉结构和回答契约之间的问题。
...
一个项目只有在公开内容、项目页面、视觉体验、QuestionPreset、Agent 回答、引用追问和 Evidence 导航全部通过验收后，才算完成。页面和 Agent 共用同一公开事实源和稳定 ID，不能各自维护重复文案或不一致的能力承诺。
...
- 首轮形成至少 2 个高质量项目的审核后内容包；目标为 3 个，但不以数量牺牲真实性。
- 每个项目至少准备一条可批准 Evidence，关键成果 Claim 具备直接支持关系。
- 每个项目的页面信息结构、核心问题和追问意图已经明确，可进入独立纵向交付批次。
- 私有原文与拟公开内容有清晰边界，脱敏结果经过用户确认。
- 候选 bundle 通过隐私检查、schema 校验和发布 dry-run；正式公开版本可以按项目批次发布。

## 6. 阶段二：逐项目交付页面视觉与 Agent

### 6.1 纵向批次

- 每次只选择一个已经审核的代表项目进入开发批次。
- 批次输入包括 Project、Claim、Evidence、TimelineEvent、QuestionPreset、页面信息结构和视觉重点。
- 批次输出必须同时包含项目页面、证据导航、Agent 核心回答、引用式追问和相应自动化测试。
- 一个项目验收完成后再开始下一个项目，避免前端和 Agent 长期处于不同内容版本。
...
- 为当前项目发布 3 至 5 个面试官核心 QuestionPreset，覆盖概览、职责、架构、取舍、验证和复盘中的高价值问题。
...
- 项目目录在数据规模足够时支持按技术、问题类型、状态和贡献类型筛选；不展示空洞筛选控件。
...
```mermaid
flowchart LR
    A["真实内容与人工审核"] --> B1["项目 1：页面 + Agent"]
    B1 --> B2["项目 2：页面 + Agent"]
    B2 --> B3["项目 3：页面 + Agent（可选）"]
    B2 --> C["整体主路径与跨项目整合"]
    B3 --> C
    C --> D["质量与可信度收口"]
    D --> E["可选智能能力评估"]
...
- 阶段二依赖阶段一的审核后内容，并按项目依次形成页面与 Agent 纵向闭环。
- 阶段三至少依赖两个已完成纵向验收的项目，第三个项目可以在不阻塞整合的情况下后续加入。
...
2. 代表项目一：公开数据、页面视觉、Agent 问答与证据闭环；
3. 代表项目二：公开数据、页面视觉、Agent 问答与证据闭环；
4. 可选代表项目三：公开数据、页面视觉、Agent 问答与证据闭环；
5. 首页、项目目录、时间线、Evidence 和跨项目 Agent 整合；
6. 无障碍、响应式、可靠性和发布门禁收口；
7. 本地检索/外部模型启用评估；
8. 云服务器 Docker 部署。

每个子项目必须单独完成设计复核、测试驱动实施和新鲜验证。项目纵向批次内部不得再把“前端”和“Agent”拆成相互等待的长期分支。第一项应在用户完成本地知识库同步后启动；在此之前可以准备内容抽取模板和审核标准，但不能虚构候选项目或公开材料。
