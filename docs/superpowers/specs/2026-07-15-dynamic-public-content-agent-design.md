...
**日期：** 2026-07-15
**状态：** 历史总设计；部分前置能力已实现，未实现范围已由 A/B/C 与 `docs/05-06` 取代，不再作为直接实施依据
**适用项目：** `D:\code\agent`
...
> **2026-07-21 权威关系：** 本文保留问题分析、演进背景和历史方案。运行时可信度与严格内存会话以 A 为准；Claim/Evidence、治理 CLI、审核和发布以 B、`docs/05`、`docs/06` 为准；模型、检索、工具、Hook 与多轮以 C 为准。本文中的 7 天浏览器会话、携带原问题和有限历史上下文的旧请求方式、旧 Claim 字段、旧回答状态、旧 hash 名称以及固定路线数字均不再具有契约效力。已实现的前端壳层和 Timeline/API 事实仍由相应已实施设计记录。
...
现有回答引擎仍只支持规范问题及其精确别名，回答固定为背景、职责、方案、验证和状态五段。后续前端重构与公开内容联调已经实现动态数量、四种身份 UI、推荐问题 UI、时间线、证据中心、完整 Agent 工作台和聚合 API；但身份没有进入回答请求，推荐问题多数不在后端问题目录中，Claim、外部版本发布、RAG、模型表达和安全回滚仍未实现。
...
- 过滤不可公开或不存在的关联；
- 返回 Service 自有查询模型，由 Controller 边界的 Mapper 映射 DTO。

Service 不得返回 Response DTO，Controller 不得直接读取仓储或文件。
...
供应商协议、base URL 和模型名通过 Answer Adapter 与环境配置提供。Domain、Service、Engine 和 Gateway 不依赖特定厂商 SDK。
...
先处理现有 `portfolio.service -> portfolio.dto.response` 反向依赖、Answer 跨模块类型泄漏、一次查询读取两次快照和隐私门禁缺口。动态扩展应建立在闭合的 Repository、Service、Gateway 和 Adapter 边界之上。

### 阶段 1：外部版本化发布包（未实施，仍待审批）

保留当前事实结构，先实现外部 active 版本、启动校验、服务器发布、重启和回滚。此阶段不接模型。

### 阶段 2：Claim、TimelineEvent 与展示编排（部分：Timeline/API 已有，Claim 与发布流程未实施）

扩展公开模型、API 和计算指标；建立私有候选审核包与公开包生成流程。当前项目内容迁移为 Claim 和 Evidence 关联。

### 阶段 3：混合检索与模型表达（未实施，仍待审批）

增加公开 RAG Chunk、关键词与向量索引、EmbeddingProvider、AnswerModelProvider、引用校验和确定性兜底。

### 阶段 4：实现目标首页和完整 Agent（前端壳层已实现，身份化回答与内容扩展未实现）
