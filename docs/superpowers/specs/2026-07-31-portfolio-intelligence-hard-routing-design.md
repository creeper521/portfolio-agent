...
- 明确 Agent 对话中结构化推荐的前端交互和数据契约，供前端开发独立实施。

## 3. 非目标

- 不建设 Selection 页面或第二套用户流程。
- 不在本实施范围内编写前端代码；前端部分只交付方案、契约和验收标准。
...
前端不建设新页面，而是在现有 Agent 对话消息中消费 `portfolioRecommendation`。

### 10.1 变更位置

前端开发需要在现有 Agent 功能内完成：
...
不得新增 Selection 路由、导航入口、页面或独立请求模块。
...
推荐卡片上的操作不调用专用 Selection 接口，也不在浏览器执行选择逻辑。操作只生成自然语言并继续调用 `/api/v2/answers`，例如：
...
- 无 `portfolioRecommendation`：保持当前普通回答渲染，不显示空容器。
- 推荐有结果：先显示 Agent 解释，再显示推荐卡片。
- 推荐为空：显示后端返回的未满足约束和 Agent 的放宽建议，不制造占位作品。
- `degraded=true`：沿用现有降级提示，并继续显示后端实际返回的结构化结果。
- 结构化字段格式非法：记录前端诊断并只显示可信的文本回答，不在客户端猜测或补齐推荐。

### 10.5 前端测试与交接验收

前端交接至少包含以下验收用例：

- 响应映射完整保留推荐结构且不共享可变数组引用。
- 非推荐回答不渲染推荐容器。
- 多个推荐项保持后端顺序。
- 空推荐正确显示未满足约束。
...
该部分由前端开发独立实现。本项目后端实施需要提供稳定的示例响应、字段说明和联调测试数据，但不修改前端源码。
...
10. 全部契约、回归和端到端测试通过。
11. 后端交付前端可直接使用的字段说明、示例响应和联调数据。
12. 本后端实施不修改前端源码，前端按第 10 节独立交付。
...
8. 交付前端字段说明、示例响应和联调数据。
9. 更新文档、Benchmark 和后端端到端测试。
