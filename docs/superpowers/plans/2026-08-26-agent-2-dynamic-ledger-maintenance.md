...
保留 A2-15、A2-20—A2-88、A2-91—A2-117；删除 Task 2 指定的 19 个已关闭 ID。可以按共同根因分组展示，但每个保留 ID 必须可检索，并拥有严重度、执行序、状态、现状证据、修复边界/依赖和专属验证/Exit Gate。
...
仅对实际命中的 A2-02、A2-16、A2-86 等已删除 ID 增加“历史缺陷 ID，已从动态账本移除”语义；未删除 A2-86 保持当前开放引用，不得误标。
