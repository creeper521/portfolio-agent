...
> **当前状态（2026-08-03）：** 历史阶段审核。模块化单体结论继续有效；本文记录的前端、package、JAR、Playwright、聚合 API 和 Timeline 延期项已在后续前端重构与公开内容联调中完成。动态发布、Claim、RAG、受约束模型表达与 PostgreSQL/pgvector 双库随后均已按独立设计实现，其中模型、检索和数据库仍默认关闭且未部署；认证与 SSE 仍未实施。
...
`StableQuestionNormalizer` 位于 `common.text`，作为 Portfolio 发布校验与 Answer Preset 匹配共同使用的无状态纯函数；`answer.engine` 不依赖该共享实现。`DeterministicAnswerEngine` 只接收 `AnswerKnowledge`，不读取 Repository，也不知道 Portfolio 类型。`AnswerResult` 持有 Answer 自己的 `AnswerEvidence`，HTTP 层再映射为 `AnswerEvidenceResponse`。
...
| 稳定问题归一化 | `common.text.StableQuestionNormalizer` |
...
本文件保留当时的模块化单体审核结论，不回写未来事项为历史事实。当前 Answer Engine 边界检查、路由职责集中度、结构化主体失败关闭顺序和治理脚本维护性等后续观察，见 [`12-工程质量与未来优化评审备忘录.md`](12-工程质量与未来优化评审备忘录.md)。其中任何一项只有转为正式设计、实现并重新通过架构门禁后，才构成对本审核结论的更新。
