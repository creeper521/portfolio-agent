...
当前仓库的每个任务先执行一次轻量分类，但不默认加载完整架构上下文：

- `NOT_APPLICABLE` 表示任务不影响 Agent 行为、架构、合同、状态、API、生产权威或架构治理，输出一行分类后立即继续，不读取架构长文档、不运行状态校验；
- `LEVEL_1` 与 `LEVEL_2` 声明边界后直接实施，不等待重复架构确认；
- 已批准的 `LEVEL_3` 直接继续 Replacement Slice，不重复申请同一批准；
- 只有边界不确定、需要改变生产权威或出现架构阻力证据时，才加载完整 Guardian 工作流。

Bootstrap 是路由，不是审批。除未经批准的生产权威变更和隐私越界外，Agent 应继续诊断、验证、隔离实验和其他范围内工作。

### Guardian 漂移的最小处理

Guardian 规则与更新的代码、通过的测试或已批准设计冲突时，只把冲突规则临时降为 advisory，并继续范围内工作。登记一个 `GUARDIAN_DRIFT` 到现有 `deferredItems`，记录冲突规则、仓库证据、Skill 更新动作与 `recheckBy`；修复仓库 Skill、补失败回归测试并同步个人安装版后关闭该项。

不增加独立漂移账本、`STALE/ADVISORY` 生命周期或每次对话哈希检查。隐私边界、外部操作授权和证据先于完成声明不参与降级。
