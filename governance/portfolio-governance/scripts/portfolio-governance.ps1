...
function Get-ExpectedAssetIds([int]$AssetCount = 68) {
    $ids = @()
...
function Read-DecisionLedger([string]$LedgerPath) {
    ...
    $assetCount = $actualIds.Count
    $expectedIds = if ($assetCount -in @(68, 72)) {
        @(Get-ExpectedAssetIds $assetCount)
    } else {
        @()
    }
    if ($assetCount -notin @(68, 72) -or
            @($actualIds | Select-Object -Unique).Count -ne $assetCount -or
        ...
        Write-Failure 'DECISION_LEDGER_ID_COVERAGE_INVALID' 'Decision ledger must cover exactly a supported 68- or 72-asset ID set.'
    }
    foreach ($asset in @($ledger.assets)) {
...
function Invoke-PrepareCandidateV3 {
    ...
    $baseVersion = [string]$portfolio.contentVersion
    if ($baseVersion -ne '2026-07-29.1' -or
        ...
        Write-Failure 'PREPARE_RUNTIME_BUNDLE_INVALID' `
            'ABTest incremental preparation requires the reviewed 2026-07-29.1 schema 4.0 Bundle.'
    }
...
if ($Command -eq 'rollback') {
    ...
    Write-Utf8Text $activeTemporary $TargetVersion
...
if ($Command -eq 'publish') {
    ...
    if (-not $Confirm) {
        ...
        $activeTemporary = Join-Path $resolvedReleaseRoot ('active.tmp.' + [guid]::NewGuid().ToString('N'))
        Write-Utf8Text $activeTemporary ([string]$portfolio.contentVersion)
        Move-Item -LiteralPath $activeTemporary -Destination (Join-Path $resolvedReleaseRoot 'active') -Force
        if (@($PostSwitchProbeUri).Count -gt 0) {
            try {
                ...
                Write-Utf8Text $restoreTemporary $previousActiveVersion
